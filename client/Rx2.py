...
def main():
    ...
    try:
        ...
        with open(results_filename, "a") as f:
            f.write(f"{datetime.now()}: RX2 Pilot phase round 1: {phi1:.6f}\n")
            f.write(f"{datetime.now()}: RX2 Pilot phase round 2: {phi2:.6f}\n")
        logger.info("Measurement results saved to %s", results_filename)

        with open("phase_saveRX2.csv", "a", newline="") as csvfile:
            csv_writer = csv.writer(csvfile)
            # 如果文件第一次写入，可以先写入表头
