...
def main():
    ...
    results_filename = "measurement_resultsRX3.txt"
    all_results = []
    try:
        ...
        with open(results_filename, "a") as f:
            f.write(f"{datetime.now()}: RX3 Pilot phase round 1: {phi1:.6f}\n")
            f.write(f"{datetime.now()}: RX3 Pilot phase round 2: {phi2:.6f}\n")
        logger.info("Measurement results saved to %s", results_filename)

        with open("phase_saveRX3.csv", "a", newline="") as csvfile:
            csv_writer = csv.writer(csvfile)
            # 如果文件第一次写入，可以先写入表头
